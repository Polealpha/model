...
\subsection{Tri-Modal Care Relation Model}
On top of the tri-modal encoder, we train a second relation model for the question raised in the advisor meeting: given multimodal affect cues and a persona prior, does this person currently need care, and if so should the system intervene now or later? This model reuses the pretrained tri-modal encoders, adds a binary care-need head, a three-way care-timing head, and continuous support-need regression, while retaining auxiliary emotion, valence-arousal, and latent-state supervision during finetuning.
