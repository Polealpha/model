...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the structured timing baseline.")
    ...
    parser.add_argument("--model-out", default=REPO_ROOT / "training" / "artifacts" / "structured_baseline.pkl")
    parser.add_argument("--metrics-out", default=REPO_ROOT / "reports" / "structured_baseline_metrics.json")
...
def eval_split(model: StructuredBaseline, episodes) -> dict:
    ...
    return {
        ...
        "strategy_accuracy": float(accuracy_score(strategy_gold, strategy_pred)),
        "strategy_macro_f1": float(f1_score(strategy_gold, strategy_pred, average="macro")),
        "timing_per_class": {
            key: {
...
def main() -> int:
    ...
    model = StructuredBaseline(cfg.get("structured_baseline", {}))
    ...
    payload = {
        "model": "structured",
        "metrics": metrics,
        "config": cfg.get("structured_baseline", {}),
        "train_sources": {
