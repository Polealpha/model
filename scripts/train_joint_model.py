...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the temporal joint timing-to-strategy model.")
    ...
    parser.add_argument("--model-out", default=REPO_ROOT / "training" / "artifacts" / "joint_model.pt")
    parser.add_argument("--metrics-out", default=REPO_ROOT / "reports" / "joint_metrics.json")
...
def eval_split(model: JointModel, episodes) -> dict:
...
def main() -> int:
    ...
    model = JointModel(cfg.get("joint", {}))
    ...
    payload = {
        "model": "joint_temporal",
        "metrics": metrics,
        "config": cfg.get("joint", {}),
        "train_sources": {
